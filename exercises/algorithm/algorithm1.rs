        ...
        while list_a.start.is_some() || list_b.start.is_some() {
            if list_b.start.is_none()
                || (list_a.start.is_some()
                    && unsafe { list_a.start.unwrap().as_ref().val.clone() }
                        < unsafe { list_b.start.unwrap().as_ref().val.clone() })
            {
                merged.add(unsafe { list_a.start.unwrap().as_ref().val.clone() });
                list_a.start = unsafe { list_a.start.unwrap().as_ref().next };
            } else {
                merged.add(unsafe { list_b.start.unwrap().as_ref().val.clone() });
